...
with st.sidebar:
    selected = option_menu(None, ["Home","Map","Classified Map","SplitMap","Requetes ","Timelapes","timeseries","slider","Explore COG","Contact"], 
                       ...
                       styles={"container": {"padding": "0!important", "background-color": "#ffeda0"},
        "icon": {"color": "#feb24c", "font-size": "25px"}, 
        "nav-link": {"font-size": "25px", "text-align": "left", "margin":"0px", "--hover-color": "#eee"},
        "nav-link-selected": {"background-color": "#f03b20"},
    })
...
if selected == "Classified Map":
    ...
    geojson_path = "https://fatielkadd.github.io/maroc/maroc1.geojson"
    ...
    if option == "PhenomeneJour-":
        ...
        m = leafmap.Map(locate_control=True, latlon_control=True, draw_export=True, minimap_control=True,zoom_start=10)
        ...
        m.add_gdf(geojson_data, layer_name="Regions")
...
# Ajoutez des blocs conditionnels pour les autres pages si nécessaire
    ...
    if selected1=="requétes attributaire":
        ...
        filter_type = st.radio("Choisir le type de filtrage", ["Par valeur", "Par intervalle de valeur"])

        if filter_type == "Par valeur":
            # Filtrer uniquement les colonnes numériques
            numeric_columns = gdf.select_dtypes(include='number').columns
            filter_column = st.selectbox("Choisir une colonne numérique pour le filtre", numeric_columns)

            # Choisissez la valeur pour le filtre
            filter_value_input = st.text_input(f"Valeur pour {filter_column}", str(gdf[filter_column].min()))
            ...
            min_value_input = st.text_input(f"Valeur minimale pour {filter_column}", str(gdf[filter_column].min()))
            max_value_input = st.text_input(f"Valeur maximale pour {filter_column}", str(gdf[filter_column].max()))
